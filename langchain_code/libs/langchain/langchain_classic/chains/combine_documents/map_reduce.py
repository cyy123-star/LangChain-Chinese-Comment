...
class MapReduceDocumentsChain(BaseCombineDocumentsChain):
    """通过先对文档应用链，然后合并结果来合并文档。

    我们首先对每个文档单独调用 `llm_chain`，传入 `page_content` 和任何其他参数。这是 `map` 步骤。

    然后，我们在 `reduce` 步骤中处理该 `map` 步骤的结果。这通常应该是一个 `ReduceDocumentsChain`。

    注意：此类已被弃用，建议查看迁移指南以获取推荐的替代方案。
    ...
    示例:
        ...
        # 如果我们愿意，还可以传入 collapse_documents_chain，
        # 它专门用于在最终调用之前折叠文档。
        prompt = PromptTemplate.from_template("折叠以下内容：{context}")
        llm_chain = LLMChain(llm=model, prompt=prompt)
    ...
    """用于合并应用 `llm_chain` 到每个文档后的结果的链。
    这通常是 ReduceDocumentChain 或 StuffDocumentChain。"""
    ...
    def combine_docs(
        ...
        **kwargs: Any,
        ...
        首先对所有文档应用第一个链进行映射，然后合并结果。
        如果需要（如果有许多文档），这种合并可以递归进行。
        """
        ...
        result_docs = [
            ...
            # 这使用来自文档的元数据，以及来自 `results` 的文本结果
