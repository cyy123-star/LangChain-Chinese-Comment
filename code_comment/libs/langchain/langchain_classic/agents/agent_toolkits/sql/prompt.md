...
此文档提供了 `libs\langchain\langchain_classic\agents\agent_toolkits\sql\prompt.py` 文件的详细中文注释。该模块定义了 SQL 代理使用的核心提示词模板常量。

## 核心说明：动态重定向

与 `agent_toolkits` 中的其他模块类似，此 file 不再直接持有提示词内容，而是通过动态导入机制重定向至 `langchain_community`。
...
- **`SQL_PREFIX`**: 
    *   **用途**: SQL 代理的基础系统提示词前缀。
    *   **内容**: 它定义了代理作为 SQL 专家的身份，并规定了其在操作数据库时的基本准则（例如：仅使用提供的工具、不要执行 DML 数据操作语言、在生成 SQL 后务必检查语法、限制返回结果行数等）。
- **`SQL_SUFFIX`**: 
    *   **用途**: 用于标准 ReAct 风格 SQL 代理的提示词后缀。
    *   **内容**: 通常包含思考路径（Thought/Action/Observation）的引导和最终回答的格式要求。
- **`SQL_FUNCTIONS_SUFFIX`**: 
    *   **用途**: 专门用于支持 OpenAI 函数调用（Function Calling）的 SQL 代理。
    *   **内容**: 针对聊天模型优化的后缀，引导模型如何根据工具返回的结果生成最终回复。
